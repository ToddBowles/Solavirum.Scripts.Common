...
Describe "New-OctopusDeployment" {
    Context "When supplied server does not exist" {
        It "Throws error with appropriate exit code as recieved from Octopus" {
            try
            {
                $result = New-OctopusDeployment -OctopusServerUrl "http://thisdoesnotexist.com:8084" -OctopusApiKey "THISAPIKEYISINVALID" -ProjectName "Non-existent Project" -Environment "CI"
            }
            catch
            {
